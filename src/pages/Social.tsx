...
import { Share2, RefreshCw, Search, Sparkles, Loader2, UserPlus } from 'lucide-react'
      ...
      <div className="md:hidden flex p-1 rounded-2xl bg-muted/60 border-2 gap-1">
        <button
          ...
          className={`flex-1 py-2 rounded-xl text-xs font-black transition-all flex items-center justify-center gap-1.5 ${
            activeTab === 'feed' ? 'bg-[#58CC02] text-white shadow-sm' : 'text-muted-foreground'
          }`}
        >
        ...
        <button
          ...
          className={`flex-1 py-2 rounded-xl text-xs font-black transition-all flex items-center justify-center gap-1.5 ${
            activeTab === 'discover' ? 'bg-[#1CB0F6] text-white shadow-sm' : 'text-muted-foreground'
          }`}
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-start">
        ...
        <div
          className={`md:col-span-7 lg:col-span-8 space-y-6 ${
            activeTab === 'discover' ? 'hidden md:block' : 'block'
          }`}
        >
          {/* Composer */}
          <PostComposer />
              ...
              <div className="bg-card rounded-3xl border-2 p-8 text-center space-y-4">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-muted-foreground font-semibold max-w-sm mx-auto">
                    Publique seu primeiro lembrete ou explore pessoas na comunidade para acompanhar
                    suas fotos e mensagens.
                  </p>
                </div>
                <Button
                  onClick={() => setActiveTab('discover')}
                  className="rounded-2xl h-11 px-5 font-black text-xs bg-[#58CC02] hover:bg-[#58CC02]/90 border-b-4 border-[#46A302] text-white"
                >
                ...
                {hasMoreFeed && (
                  <div className="text-center pt-2">
                    <Button
                      ...
                      disabled={loadingFeed}
                      variant="outline"
                      className="rounded-2xl h-11 px-6 font-bold border-2 text-xs hover:bg-muted"
                    >
                      {loadingFeed ? (
                        <>
