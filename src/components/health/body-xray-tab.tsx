...
export function BodyXrayTab() {
  ...
  const bmi = useMemo(() => {
    if (!latest || !patientGoals.height) return 0
    const h = patientGoals.height / 100
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="font-extrabold">
                {v?.hr || '—'} <span className="text-xs text-muted-foreground font-bold">bpm</span>
              </span>
            </div>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
              ...
              <div className="flex items-center justify-between mb-1">
                ...
                <div className="flex gap-0.5">
                  {[1, 2, 3, 4, 5].map((n) => (
                    <span
                      key={n}
                      className={cn('text-sm', (v?.sleep || 0) >= n ? 'opacity-100' : 'opacity-20')}
                    >
                      ⭐
                    </span>
                  ))}
