    ...
    if (emptyPlot !== -1) {
      ...
      if (growingPlant) {
        ...
        toast({
          ...
          description:
            'Todos os seus canteiros estão cheios! Colha suas flores ou desbloqueie mais canteiros!',
        })
      }
    }
  },
  ...
  waterPlant: async (plotIndex: number) => {
    ...
    if (target.stage >= 3) {
      toast({
        title: 'Flor radiante! ✨',
        description:
          'Esta flor já atingiu seu potencial máximo! Você pode colhê-la para liberar espaço.',
      })
      return
    }
